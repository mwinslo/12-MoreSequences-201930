...
def run_test_generate_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    expected = [rg.Point(101.0, 100.0),  # All numbers are approximate.
                rg.Point(100.0, 101.0),
                rg.Point(99.0, 100.0),
                rg.Point(100.0, 99.0),
                ]
    circle = rg.Circle(rg.Point(100, 100), 1)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    circle.attach_to(window)
    for k in range(len(points)):
        cut = rg.Line(circle.center, points[k])
        cut.color = color
...
def run_test_polygon():
    ...
    circle = rg.Circle(rg.Point(200, 200), 50)
    circle.outline_thickness = 3
    polygon(window, circle, 5, 'magenta', 10)
...
def run_test_fancy_polygon():
    ...
    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'blue'
    fancy_polygon(window, circle, 20, 7, 'orange', 3)
    window.close_on_mouse_click()
