...
def run_test_make_simple_string():
    ...
    actual = make_simple_string(2, 2)
    print("Expected: 2-")
    print("Actual:  ", actual)

    actual = make_simple_string(5, 9)
    print("Expected: 5-6-7-8-9-")
    print("Actual:  ", actual)
...
def make_simple_string(m, n):
    ...
    string = str("{}-".format(m))
    for k in range(m, n):
        string = string + str("{}-".format(str(k + 1)))
    return string
...
def rectangles_from_circles(circles):
    ...
    for k in range(len(circles)):
        ...
        rectangle = rg.Rectangle(rg.Point(circle.center.x - radius, circle.center.y - radius), rg.Point(circle.center.x + radius, circle.center.y + radius))
